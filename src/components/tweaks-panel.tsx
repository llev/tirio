'use client';
// tweaks-panel.tsx — reusable Tweaks shell + form-control helpers (ported from tweaks-panel.jsx).
// The prototype-specific host protocol (window.parent.postMessage) has been removed.

import { useState, useEffect, useRef, useCallback } from 'react';

const __TWEAKS_STYLE = `
  .twk-panel{position:fixed;right:16px;bottom:16px;z-index:2147483646;width:280px;
    max-height:calc(100vh);display:flex;flex-direction:column;
...
// ── useTweaks ───────────────────────────────────────────────────────────────
// Single source of truth for tweak values. (Host persistence removed in the port.)
...
export function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  // Keyboard shortcut to open/close the panel since the host toggle is gone.
  useEffect(() => {
    const onKey = (e) => {
      if (e.altKey && (e.key === 't' || e.key === 'T')) setOpen((o) => !o);
    };
